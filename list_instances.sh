...
if [ $STATE = "running" ]; then
	...
	aws ec2 describe-instances --query 'Reservations[*].Instances[*].[InstanceId,Tags[?Key==`Name`].Value | [0], InstanceType, LaunchTime, State.Name]' --filters Name=instance-state-name,Values=$STATE --output table --region ap-southeast-2
fi
